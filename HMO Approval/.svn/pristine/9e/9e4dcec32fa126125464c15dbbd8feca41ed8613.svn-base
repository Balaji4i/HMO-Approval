...
public class ManageHmoBB {
    ...
    private String doApproveAction(String status) {
        ...
        if("APPROVED".equals(status)){
            ...
            r.setAttribute("ApprovalStatus", "REJECTED");
        }
        ...
        String approvalStatus = r.getAttribute("ApprovalStatus")!=null?r.getAttribute("ApprovalStatus").toString():"";
        ...
        xmlData = ApprovalPayload.hmoApprovalXMLData(reqNo, reqDate, empNo, empName, email, buGrp, approvalStatus, comments, 
    ...
    public void onClickExEmpAttachment(ActionEvent actionEvent) {
        String dtlId = ADFUtils.evaluateEL("#{bindings.HmoHdrId.inputValue}") == null ? "0" :
                    ADFUtils.evaluateEL("#{bindings.HmoHdrId.inputValue}").toString();
        System.err.println("-dtlId->"+dtlId);
        ViewObject attachVo = com.view.uiutils.ADFUtils.findIterator("XxExEmpAttachment_EOView1Iterator").getViewObject();
        attachVo.applyViewCriteria(null);
        attachVo.executeQuery();
        ViewCriteria vc = attachVo.createViewCriteria();
        ViewCriteriaRow vcRow = vc.createViewCriteriaRow();
        vcRow.setAttribute("SourceDocumentId", dtlId);
        vc.addRow(vcRow);
        vcRow.setAttribute("SourceDocument", "Ex_HMO_Employee");
        vc.addRow(vcRow);
        attachVo.applyViewCriteria(vc);
        attachVo.executeQuery();
        JSFUtils.showPopup(attachPop);
    ...
    public void onDownloadExEmployee(FacesContext facesContext, OutputStream outputStream) {
        ViewObject vc = ADFUtils.findIterator("XxExEmpAttachment_EOView1Iterator").getViewObject();
        BlobDomain blob = (BlobDomain) vc.getCurrentRow().getAttribute("Attachment");
        try {
            IOUtils.copy(blob.getInputStream(), outputStream);
            blob.closeInputStream();
